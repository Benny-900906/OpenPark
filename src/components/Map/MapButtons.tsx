...
export const MapCenterButton = ({ position }: {position: [number, number]}) => {
  ...
  return (
    <button className="bg-black p-2 rounded shadow" onClick={() => { map.setView(position, map.getZoom()); }}>
...
export const MapUserLocationButton = () => {
  ...
  return (
    <button className="bg-black p-2 rounded shadow" onClick={handleUserCurrentLocation}>
      <img src={userLocationIcon} alt="icon" className="w-6 h-6" />
    </button>
    );
}